...
async def on_member_join(member):
    ...
    print(
        f'[{date_object.strftime("%H:%M:%S - %b %d %Y")}] [ Roundtrip: {round(client.latency * 1000)}ms.] CONSOLE: JOIN | LOG: Member {member} has joined the server!')
...
async def on_member_remove(member):
    date_object = datetime.now()
    print(
        f'[{date_object.strftime("%H:%M:%S - %b %d %Y")}] [ Roundtrip: {round(client.latency * 1000)}ms.] CONSOLE: LEAVE | LOG: Member {member} has left the server!')
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        await ctx.reply(
            'Whoops, looks like I can not find that command! Try inputting it correctly, and remember, it can be case sensitive!')
